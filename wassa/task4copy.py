...
if __name__ == "__main__":
    ...
    #load data
    train_df = pd.read_csv('data/training/sadness-ratings-0to1.train.txt', sep='\t', header=0)
    test_df = pd.read_csv('data/validation/sadness-ratings-0to1.dev.gold.txt', sep='\t', header=0)
    ...
    checkpointer = callbacks.ModelCheckpoint(filepath="logs/sadness/checkpoint-{epoch:02d}.hdf5", verbose=1, save_best_only=True, monitor='loss')
    csv_logger = CSVLogger('logs/sadness/training_set_iranalysis1.csv',separator=',', append=False)
