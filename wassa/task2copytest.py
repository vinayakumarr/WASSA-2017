...
if __name__ == "__main__":
    ...
    test_df = pd.read_csv('data/testing/fear-ratings-0to1.test.target.txt', sep='\t', header=0)
    ...
    model.load_weights("logs/fear/checkpoint-77.hdf5")
    print("---------------------------------------")
    print(word_id_test)
    testPredict = model.predict(word_id_test)
    np.savetxt("fear.txt", testPredict, fmt="%1.3f")

    #for file in os.listdir("logs/fear/"):
    #   model.load_weights("logs/fear/"+file)
    #   print("---------------------------------------")
    #   print(file)
    #   testPredict = model.predict(word_id_test)
