...
if __name__ == "__main__":
    ...
    test_df = pd.read_csv('data/testing/sadness-ratings-0to1.test.target.txt', sep='\t', header=0)
    ...
    model.load_weights("logs/sadness/checkpoint-155.hdf5")
    print("---------------------------------------")
    print(word_id_test)
    testPredict = model.predict(word_id_test)
    np.savetxt("sadness.txt", testPredict, fmt="%1.3f")
    #trainScore = math.sqrt(mean_squared_error(y_test, testPredict))
    #print('Test Score: %.2f RMSE' % (trainScore))
    #print(testPredict.shape)
    #for i in np.nditer(testPredict):
    #  print(i)
       

    #for file in os.listdir("logs/sadness/"):
    #   model.load_weights("logs/sadness/"+file)
    #   print("---------------------------------------")
    #   print(file)
    #   testPredict = model.predict(word_id_test)
    #   trainScore = math.sqrt(mean_squared_error(y_test, testPredict))
